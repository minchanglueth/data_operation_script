...
def get_ituneid(gsheet_url: str):
    ...
    worksheet = sh.worksheet("Test_1")
    df = get_as_dataframe(worksheet, header=1, skip_blank_lines=True, evaluate_formulas=True)
...
def update_albums(id_list, df):
    ...
    for row in merged_df.index:
        ...
        db_session.query(Album).filter(Album.uuid == new_uuid).update(
            {Album.info: info}, synchronize_session="fetch"
        )
...
if __name__ == "__main__":
    ...
    print(id_list)
    time.sleep(900)
    update_albums(id_list=id_list, df=df)
